        ...
        public bool mtdUpdateControlPropiedad(ref clsControlPropiedadCP objPrograma, ref string strErrMsg)
        {
            ...
            try
            {
                strConsulta = string.Format("UPDATE [Procesos].[tblControlPropiedadClienteProveedor]  SET [Descripcion] = '{0}',[Caracteristicas] = '{1}', " +
                    "[ProveedorCliente] = '{2}',[FechaIngreso] = '{3}',[FechaSalida] = '{4}',[Observaciones] = '{5}'," +
                    "[FechaRegistro] = GETDATE(),[IdUsuario] = {7}, [Nombre] = '{9}'" +
                    " WHERE IdControlPropiedadClienteProveeedor = {8} ",
                    objPrograma.strDescripcion, objPrograma.strCaracteristicas, objPrograma.strProveedorCliente, objPrograma.dtFechaIngreso,
                    objPrograma.dtFechaSalida, objPrograma.strObservaciones, "GETDATE()"/*objPrograma.dtFechaRegistro*/, objPrograma.intIdUsuario,
                    objPrograma.intIdCrlPropiedad, objPrograma.strNombre);
