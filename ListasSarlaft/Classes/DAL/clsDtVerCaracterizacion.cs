...
namespace ListasSarlaft.Classes
{
    public class clsDtVerCaracterizacion : IDisposable
    {
        ...
        public bool mtdConsultarVerCaracterizacionIndicador(clsVerCaracterizacion objCaracIn, ref DataTable dtCaracOut, ref string strErrMsg)
        {
            ...
            try
            {
                //strConsulta = string.Format("SELECT DISTINCT [NombreIndicador],[Descripcion],ISNULL([IdIndicador], '')IdIndicador FROM [dbo].[vwVerCaracterizacionIndicadorRiesgo] " +
                strConsulta = string.Format("SELECT DISTINCT PTI.NombreIndicador,PTI.Descripcion,ISNULL(PTPI.Id, '')IdIndicador " +
                    "from Procesos.tblIndicador PTI INNER JOIN Procesos.tblProcesoIndicador PTPI ON PTI.IdProcesoIndicador = PTPI.Id" +
                    " LEFT OUTER JOIN Procesos.tblCaracterizacion PP ON PP.IdProceso = PP.IdProceso" +
                    //"where IdProcesoIndicador = {0}",
                    //"where IdProceso = {0} and IdCadenaValor = {1}",
                    " where PP.IdProceso = {0}",
                    //objCaracIn.intIdProceso, objCaracIn.intIdUsuario);
                    objCaracIn.intIdProceso);
        ...
        public string mtdConsultaCadenaValor(String nomCadenaValor)
        {
            #region Vars
            string strConsulta = string.Empty;
            ...
            try
            {
                strConsulta = string.Format("select IdCadenaValor from Procesos.CadenaValor where NombreCadenaValor = '{0}'",
                    nomCadenaValor);

                cDataBase.conectar();
                DataTable dt = cDataBase.ejecutarConsulta(strConsulta);
        ...
        public string mtdConsultaMacroproceso(String nomMacroProceso)
        {
            #region Vars
            string strConsulta = string.Empty;
            string rt = string.Empty;
            ...
            try
            {
                strConsulta = string.Format("select IdMacroProceso from Procesos.Macroproceso where Nombre = '{0}'",
                    nomMacroProceso);
        ...
        public bool mtdConsultarVerCaracterizacionRiesgos(clsVerCaracterizacion objCaracIn, ref DataTable dtCaracOut, ref string strErrMsg)
        {
            ...
            try
            {
                ...
                strConsulta = string.Format("SELECT DISTINCT ISNULL([Codigo],'')Codigo,ISNULL([Nombre],'')NombreRiesgo," +
                    "ISNULL([Descripcion],'')DescripcionRiesgo,ISNULL([IdRiesgo],'')IdRiesgo " +
