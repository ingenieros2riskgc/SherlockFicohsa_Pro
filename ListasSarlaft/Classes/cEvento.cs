...
namespace ListasSarlaft.Classes
{
    public class cEvento : cPropiedades
    {
        ...
        public DataTable consultarCodigo(int IdEvento)
        {
            DataTable dtInformacion = new DataTable();
            try
            {
                cDataBase.conectar();
                dtInformacion = cDataBase.ejecutarConsulta("select CodigoEvento from Riesgos.Eventos where IdEvento = " + IdEvento + "");
                cDataBase.desconectar();
            }
            catch (Exception ex)
            {
        ...
        public void relacionarPA(String IdPA, string codigoEvento)
        {
            try
            {
                cDataBase.conectar();
                cDataBase.ejecutarQuery("INSERT INTO Riesgos.PlanesEventosAsociados (CodigoPlan, CodigoEvento, FechaRegistro, Usuario) VALUES ('" + IdPA + "', '" + codigoEvento + "', GETDATE(), " + IdUsuario + ")");
                cDataBase.desconectar();
            }
            catch (Exception ex)
            {
        ...
        public void mtdModificarEvento3(string IdDesFactorRO, string IdDesCategoria, string IdDesCausaNUno, string IdDesCausaNDos, string IdEvento)
        {
            string strConsulta = string.Empty;

            try
            {
                strConsulta = string.Format($"UPDATE Riesgos.Eventos SET IdDesFactorRO='{IdDesFactorRO}',IdDesCategoria = '{IdDesCategoria}',IdDesCausaNUno='{IdDesCausaNUno}',IdDesCausaNDos='{IdDesCausaNDos}' WHERE (CodigoEvento = '{IdEvento}')");
            ...
            try
            {
                ...
                switch (numeroQuery)
                {
                    case "1":
                        ...
                        strQry1 = "SELECT a.CodigoEvento, LTRIM(RTRIM(ISNULL(em.Nombrehijo, ''))) Empresa, SUBSTRING(CONVERT(VARCHAR, a.FechaEvento, 120), 1, 10)  FechaNoHuboEvento, j.NombreResponsable, j.CargoResponsable Cargo, PAr.NombreArea Area";
                        strQry2 = "FROM Riesgos.NoHuboEventos a LEFT JOIN Listas.Usuarios l ON a.IdUsuario = l.IdUsuario LEFT JOIN Parametrizacion.DetalleJerarquiaOrg j ON l.IdJerarquia = j.idHijo LEFT JOIN Parametrizacion.JerarquiaOrganizacional jo ON j.idHijo = jo.idHijo LEFT JOIN [Parametrizacion].[JerarquiaOrganizacional] em ON a.IdEmpresa = em.idHijo LEFT JOIN [Parametrizacion].[JerarquiaOrganizacional] PJO ON l.IdJerarquia  = PJO.idHijo LEFT JOIN [Parametrizacion].[DetalleJerarquiaOrg] PDJO ON PJO.idHijo = PDJO.idHijo LEFT JOIN [Parametrizacion].[Area] PAr ON PDJO.IdArea = PAr.IdArea";
...
<<<<<<< HEAD
                        strQry1 = "SELECT a.CodigoEvsEIncs 'Código', SUBSTRING(CONVERT(VARCHAR,a.FechaRegistro, 120),1, 10) 'Fecha Registro',  a.DescripcionEvento 'Descripción Evento', SUBSTRING(CONVERT(VARCHAR,a.FechaOcurrencia, 120), 1, 10) 'Fecha Ocurrencia',  SUBSTRING(CONVERT(VARCHAR,a.FechaCierre, 120), 1, 10) 'Fecha Cierre', SUBSTRING(CONVERT(VARCHAR,a.FechaDescubrimiento, 120), 1, 10) 'Fecha Descubrimiento', a.MontoBruto 'Monto Bruto',a.Notas, a.MontoExposicion 'Monto Exposicion', a.Recuperaciones,p.CodigoPlan 'Código Plan Acción', p.DescripcionPlan 'Plan Acción', p.Responsable 'Responsable Plan Acción', p.Estado 'Estado Plan Acción', SUBSTRING(CONVERT(VARCHAR, p.FechaCompromiso, 120), 1, 10) 'Fecha Compromiso Plan Acción'";
                       // strQry2 = "FROM Riesgos.EventosIncidentes a INNER JOIN Riesgos.Planes_EvsEIncs pei ON pei.IdEvsEIncs = a.IdEvsEIncs INNER JOIN Riesgos.planes p ON p.Id = pei.IdPlanes";
=======
                        strQry1 = "SELECT a.CodigoEvsEIncs 'Código', SUBSTRING(CONVERT(VARCHAR,a.FechaRegistro, 120),1, 10) 'Fecha Registro',  a.DescripcionEvento 'Descripción Evento', SUBSTRING(CONVERT(VARCHAR,a.FechaOcurrencia, 120), 1, 10) 'Fecha Ocurrencia',  SUBSTRING(CONVERT(VARCHAR,a.FechaCierre, 120), 1, 10) 'Fecha Cierre', SUBSTRING(CONVERT(VARCHAR,a.FechaDescubrimiento, 120), 1, 10) 'Fecha Descubrimiento', a.MontoBruto 'Monto Bruto',a.Notas, a.MontoExposicion 'Monto Exposicion', a.Recuperaciones,CASE WHEN p.CodigoPlan IS NULL THEN 'Sin Plan Accion Asociado'ELSE p.CodigoPlan END 'Código Plan Acción', p.DescripcionPlan 'Plan Acción', p.Responsable 'Responsable Plan Acción', p.Estado 'Estado Plan Acción', SUBSTRING(CONVERT(VARCHAR, p.FechaCompromiso, 120), 1, 10) 'Fecha Compromiso Plan Acción'";
>>>>>>> 1cf729fcded3e0e39d801b87c751acc9a0c75691
                        strQry2 = "FROM Riesgos.Planes_EvsEIncs pei RIGHT JOIN Riesgos.EventosIncidentes a  ON a.IdEvsEIncs = pei.IdEvsEIncs LEFT JOIN Riesgos.planes p ON p.id = pei.IdPlanes";
                        strConsulta = string.Format("{0} {1} {2} ORDER BY a.CodigoEvsEIncs", strQry1, strQry2, condicion);
                        #endregion
        ...
        public DataTable ReporteEventosIncidentes(int IdAnio, string Filtro, string selectedProc)
        {
            #region Variables
            DataTable dtInformacion = new DataTable();
            #endregion Variables
            string consulta = "SELECT CASE WHEN LEN('Proceso Afectado - Título del evento') IS NULL THEN 0 ELSE LEN('Proceso Afectado - Título del evento') END as Titulo_Proceso, CASE WHEN LEN([Proceso Afectado]) IS NULL THEN 0 ELSE LEN([Proceso Afectado]) end as 'Proceso_Afectado', * FROM Riesgos.vwEventosIncidentes";
            try
            {
                cDataBase.conectar();
                ...
                {
                    where = "";
                    dtInformacion = cDataBase.ejecutarConsulta("SELECT Anio FROM Eventos.Anios WHERE IdAnio = " + IdAnio);
                    string Anio = dtInformacion.Rows[0]["Anio"].ToString();

                    if (Filtro.Equals("3"))
                    {
                        consulta += " WHERE YEAR([Fecha de Descubrimiento])<=" + Anio + " and YEAR([Fecha de cierre])>=" + Anio + " or YEAR([Fecha de Descubrimiento])<=" + Anio + " and [Fecha de cierre] is null";
                    }
                    else
                    {
                        consulta += " WHERE (FechaDescubrimiento >= CAST(N'" + Anio + "-01-01' AS DATE) AND FechaDescubrimiento <= CAST(N'" + Anio + "-12-31' AS DATE)) ";
                        if (Filtro.Equals("2"))
                        {
                            consulta += " OR (FechaRegistroPerdida >= CAST(N'" + Anio + "-01-01' AS DATE) AND FechaRegistroPerdida <= CAST(N'" + Anio + "-12-31' AS DATE))";
                        }
                    }
                }
                else
                ...
                if (selectedProc != "---")
                {
                    consulta += where + " AND [IdProcesoAfectado] = '" + selectedProc + "'";
                }
        ...
        public DataTable loadInfoEventos(String CodigoEvento, String DescripcionEvento, String CodigoPlanAccion)
        {
            DataTable dtInformacion = new DataTable();
            ...
            try
            {
                ...
                CondicionOtroFiltro = "SELECT rei.IdEvsEIncs AS IdEvento, rei.CodigoEvsEIncs AS CodigoEvento, rei.DescripcionEvento, rei.FechaRegistro, CONCAT(lu.Nombres, ' ', lu.Apellidos) AS Usuario  " +
                    "FROM Riesgos.EventosIncidentes rei INNER JOIN Listas.Usuarios lu ON lu.IdUsuario = rei.IdUsuarioRegistro ";

                CondicionOtroFiltro += " WHERE(rei.CodigoEvsEIncs <> '') " + condicion;
        ...
        public void mtdAgregarArchivoPdf(string strIdControl, string strIdRegistro, string strUrlArchivo, byte[] bPdfData)
        {
            ...
            try
            {
                strConsulta = string.Format("INSERT INTO Riesgos.Archivos (" +
                    "[IdControlUsuario], [IdRegistro], [NombreUsuario], [FechaRegistro], [UrlArchivo], [ArchivoPDF]) " +
                    "VALUES ({3}, {0}, '{1}', GETDATE(),N'{2}', @PdfData)",
