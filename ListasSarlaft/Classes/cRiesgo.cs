            ...
            try
            {
                ...
                switch (numeroQuery)
                {
                    ...
                    case "3":
                        ...
                        strSelect = "SELECT (RR.Codigo) CodigoRiesgo,  LTRIM(RTRIM(LU.Nombres)) + ' ' + LTRIM(RTRIM(LU.Apellidos)) Usuario, LTRIM(RTRIM(RR.Nombre)) NombreRiesgo, LTRIM(RTRIM(RR.Descripcion)) DescripcionRiesgo, ResponsableRiesgo.NombreHijo ResponsableRiesgo, LTRIM(RTRIM(ISNULL(CONVERT(VARCHAR, RR.FechaRegistro, 23), ''))) FechaRegistroRiesgo, LTRIM(RTRIM(ISNULL(PCR.NombreClasificacionRiesgo, ''))) ClasificacionRiesgo, LTRIM(RTRIM(ISNULL(PCGR.NombreClasificacionGeneralRiesgo, ''))) ClasificacionGeneralRiesgo, LTRIM(RTRIM(ISNULL(PCPR.NombreClasificacionParticularRiesgo, ''))) ClasificacionParticularRiesgo, LTRIM(RTRIM(ISNULL(PTRO.NombreTipoRiesgoOperativo, ''))) AS TipoRiesgoOperativo, LTRIM(RTRIM(ISNULL([Procesos].[FnDescriptionListCausas](RR.ListaCausas, '|'), ''))) Causas, LTRIM(RTRIM(ISNULL([Procesos].[FnDescriptionListConsecuencias](RR.ListaConsecuencias, '|'), ''))) Consecuencias, LTRIM(RTRIM(ISNULL(PCV.NombreCadenaValor, ''))) CadenaValor, LTRIM(RTRIM(ISNULL(PM.Nombre, ''))) Macroproceso, LTRIM(RTRIM(ISNULL(PP.Nombre, ''))) Proceso, LTRIM(RTRIM(ISNULL(PS.Nombre, ''))) Subproceso, LTRIM(RTRIM(ISNULL(PA.Nombre, ''))) Actividad, LTRIM(RTRIM(ISNULL([Procesos].[FnDescriptionListTreatment](RR.ListaTratamiento, '|'), ''))) ListaTratamiento, LTRIM(RTRIM(ISNULL(PPr.NombreProbabilidad, ''))) FrecuenciaInherente, LTRIM(RTRIM(ISNULL(PPr.ValorProbabilidad, ''))) CodigoFrecuenciaInherente, LTRIM(RTRIM(ISNULL(PIm.NombreImpacto, ''))) ImpactoInherente, LTRIM(RTRIM(ISNULL(PIm.ValorImpacto, ''))) CodigoImpactoInherente, LTRIM(RTRIM(ISNULL(PRI.NombreRiesgoInherente, ''))) RiesgoInherente, LTRIM(RTRIM(ISNULL(PRI.ValorRiesgoInherente, ''))) CodigoRiesgoInherente, LTRIM(RTRIM(ISNULL(PR.NombreProbabilidad, ''))) FrecuenciaResidual, LTRIM(RTRIM(ISNULL(PR.ValorProbabilidad, ''))) CodigoFrecuenciaResidual, LTRIM(RTRIM(ISNULL(IM.NombreImpacto, ''))) ImpactoResidual, LTRIM(RTRIM(ISNULL(im.ValorImpacto, ''))) CodigoImpactoResidual, LTRIM(RTRIM(ISNULL(RiesgoResidual.NombreRiesgoInherente, ''))) RiesgoResidual, LTRIM(RTRIM(ISNULL(RiesgoResidual.ValorRiesgoInherente, ''))) CodigoRiesgoResidual, LTRIM(RTRIM(ISNULL(REI.CodigoEvsEIncs, ''))) CodigoEvento, LTRIM(RTRIM(ISNULL(REI.DescripcionEvento, ''))) DescripcionEvento,LTRIM(RTRIM(ISNULL(CONVERT(VARCHAR, REI.FechaOcurrencia, 23), ''))) FechaOcurrencia, LTRIM(RTRIM(ISNULL(CONVERT(VARCHAR, REI.FechaDescubrimiento, 23), ''))) FechaDescubrimiento,LTRIM(RTRIM(ISNULL(CONVERT(VARCHAR, REI.FechaRegistro, 23), ''))) FechaRegistroEvento, LTRIM(RTRIM(ISNULL(PFRO.NombreFactorRiesgoOperativo, ''))) FactorRiesgoOperativo, LTRIM(RTRIM(ISNULL(PRAO.NombreRiesgoAsociadoOperativo, ''))) RiesgoAsociadoOperativo, LTRIM(RTRIM(ISNULL(PTRO.NombreTipoRiesgoOperativo, ''))) SubFactorRiesgoOperativo, LTRIM(RTRIM(ISNULL([Procesos].[FnDescriptionList](RR.LISTARIESGOASOCIADOLA, '|'), ''))) ListaRiesgoAsociadoLA, LTRIM(RTRIM(ISNULL([Procesos].[FnDescriptionListLAFT](RR.LISTAFACTORRIESGOLAFT, '|'), ''))) ListaFactorRiesgoLAFT, 'Region:' + LTRIM(RTRIM(ISNULL(PReg.NombreRegion, ''))) + ' - Pais:' + LTRIM(RTRIM(ISNULL(PPai.NombrePais, ''))) + ' - Depto:' + LTRIM(RTRIM(ISNULL(PTEO.NombreTipoEventoOperativo, ''))) TipoEvento,Parea.NombreArea, LTRIM(RTRIM(ISNULL(EF.Descripcion, ''))) FactorRO, LTRIM(RTRIM(ISNULL(ET.Tipo, ''))) TipoActivo, LTRIM(RTRIM(ISNULL(EA.Activo, ''))) ActivoAfectado, LTRIM(RTRIM(ISNULL(ED.Dimensiones, ''))) Dimensiones ";
                        strFrom = "FROM Riesgos.EventosIncidentes REI INNER JOIN Riesgos.Riesgo RR ON RR.IdRiesgo = REI.IdRiesgo LEFT JOIN Parametrizacion.ClasificacionRiesgo PCR ON RR.IdClasificacionRiesgo = PCR.IdClasificacionRiesgo LEFT JOIN Parametrizacion.ClasificacionGeneralRiesgo PCGR ON RR.IdClasificacionGeneralRiesgo = PCGR.IdClasificacionGeneralRiesgo LEFT JOIN Parametrizacion.ClasificacionParticularRiesgo PCPR ON RR.IdClasificacionParticularRiesgo = PCPR.IdClasificacionParticularRiesgo LEFT JOIN Parametrizacion.TipoRiesgoOperativo PTRO ON RR.IdTipoRiesgoOperativo = PTRO.IdTipoRiesgoOperativo LEFT JOIN Parametrizacion.TipoEventoOperativo PTEO ON RR.IdTipoEventoOperativo = PTEO.IdTipoEventoOperativo LEFT JOIN Procesos.CadenaValor PCV ON PCV.IdCadenaValor = RR.IdCadenaValor LEFT JOIN Procesos.Macroproceso PM ON RR.IdMacroproceso = PM.IdMacroProceso LEFT JOIN Procesos.Proceso PP ON RR.IdProceso = PP.IdProceso LEFT JOIN Procesos.Subproceso PS ON PS.IdSubProceso = RR.IdSubProceso LEFT JOIN Procesos.Actividad PA ON RR.IdActividad = PA.IdActividad LEFT JOIN Parametrizacion.Probabilidad PPr ON PPr.IdProbabilidad = RR.IdProbabilidad LEFT JOIN Parametrizacion.Probabilidad PR ON PR.IdProbabilidad = RR.IdProbabilidadResidual LEFT JOIN Parametrizacion.Impacto PIm ON PIm.IdImpacto = RR.IdImpacto LEFT JOIN Parametrizacion.Impacto IM ON IM.IdImpacto = RR.IdImpactoResidual LEFT JOIN Parametrizacion.RiesgoInherente PRI ON PRI.IdProbabilidad = RR.IdProbabilidad AND PRI.IdImpacto = RR.IdImpacto LEFT JOIN Parametrizacion.RiesgoInherente RiesgoResidual ON RR.IdProbabilidadResidual = RiesgoResidual.IdProbabilidad AND RR.IdImpactoResidual = RiesgoResidual.IdImpacto LEFT JOIN Parametrizacion.JerarquiaOrganizacional ResponsableRiesgo ON ResponsableRiesgo.idHijo = RR.IdResponsableRiesgo LEFT JOIN Parametrizacion.FactorRiesgoOperativo PFRO ON RR.IdFactorRiesgoOperativo = PFRO.IdFactorRiesgoOperativo LEFT JOIN Parametrizacion.RiesgoAsociadoOperativo PRAO ON RR.IdRiesgoAsociadoOperativo = PRAO.IdRiesgoAsociadoOperativo LEFT JOIN Parametrizacion.Regiones PReg ON RR.IdRegion = PReg.IdRegion LEFT JOIN Parametrizacion.Paises PPai ON RR.IdPais = PPai.IdPais AND PReg.IdRegion = PPai.IdRegion INNER JOIN Listas.Usuarios LU ON RR.IdUsuario = LU.IdUsuario LEFT JOIN [Parametrizacion].[DetalleJerarquiaOrg] AS PDJ ON PDJ.idHijo = ResponsableRiesgo.idHijo LEFT JOIN Parametrizacion.Area as Parea on Parea.IdArea = PDJ.IdArea LEFT JOIN Eventos.ActivoAfectado EA ON RR.IdActivoAfectado = EA.IdActivoAfectado  LEFT JOIN Eventos.DesFactorRO EF ON RR.IdFactorRO = EF.IdDesFactorRO LEFT JOIN Eventos.TipoActivo ET ON RR.IdTipoActivo = ET.IdTipoActivo LEFT JOIN Eventos.DimValoracion ED ON RR.IdDimensiones = ED.IdDimValoracion ";
                        strConsulta = string.Format("{0} {1} {2} ORDER BY RR.IdRiesgo", strSelect, strFrom, condicion);
