...
namespace ListasSarlaft.Classes
{
    public class cCuenta : cPropiedades
    {
        ...
        public DataTable MensajeLogin(String Usuario)
        {
            ...
            try
            {
                string query = "SELECT LTRIM(RTRIM(ISNULL(LU.Nombres,''))) + ' ' + LTRIM(RTRIM(ISNULL(LU.Apellidos,''))) AS NombreUsuario," +
                    "LU.Contrasena, LU.Bloqueado, LU.login, LU.FechaUltActualPassword, LU.IdUsuario, " +
                    "LTRIM(RTRIM(LR.NombreRol)) AS NombreRol, LTRIM(RTRIM(LU.Usuario)) AS Usuario, LR.IdRol," +
                    "LU.IdJerarquia, LU.IdMacroProcesoU, LU.IdProcesoU, LU.VerTodosProcesos " +
                    ",ISNULL(ar.NombreArea,'') AS NombreArea,ISNULL(djo.IdArea,0) AS IdArea " +
                    "FROM Listas.Usuarios LU " +
                    "INNER JOIN Listas.Roles LR ON LU.IdRol = LR.IdRol " +
                    "Left outer JOIN Parametrizacion.DetalleJerarquiaOrg djo on djo.idHijo = LU.IdJerarquia " +
"Left outer JOIN Parametrizacion.Area AS ar on ar.IdArea = djo.IdArea  " +
        ...
        public DataTable contadorMeta(string IdValor)
        {
            DataTable dtInformacion = new DataTable();
            try
            {
                cDataBase.conectar();
                dtInformacion = cDataBase.ejecutarConsulta("select SUM(Meta) as Total from Gestion.MetaValor where IdIndicador = " + IdValor + "");
                cDataBase.desconectar();
            }
            catch (Exception ex)
            {
