...
namespace ListasSarlaft.Classes
{
    public class cParametrizacionRiesgos : cPropiedades
    {
        ...
        public Boolean borrarRegistroTipoLegislacion(String IdTipoLegislacion)
        {
            Boolean flag = false;
            DataTable dtInformacion = new DataTable();
            try
            {
                cDataBase.conectar();
                dtInformacion = cDataBase.ejecutarConsulta("SELECT NombreLegislacion  FROM [Riesgos].[Legislacion] where IdTipoLegislacion = " + IdTipoLegislacion);
                cDataBase.desconectar();
                if (dtInformacion.Rows.Count > 0)
                {
                    Session["NombreLegislacion"] = dtInformacion.Rows[0]["NombreLegislacion"].ToString();
                }
            }
            catch (Exception ex)
            {
