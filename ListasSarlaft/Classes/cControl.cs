...
namespace ListasSarlaft.Classes
{
    public class cControl : cPropiedades
    {
        ...
        public DataTable loadInfoControles1(String CodigoControl, String NombreControl,
            String Responsable, String Area)
        {
            DataTable dtInformacion = new DataTable();
            String condicion = "";
            try
            {
                ...
                if (Area != "---")
                {
                    if (condicion.Trim() == "")
                    {
                        condicion = "WHERE PDJ.IdArea = " + Area + "";
                    }
                    else
                    {
                        condicion += "AND PDJ.IdArea = " + Area + "";
                    }
                }
                cDataBase.conectar();
                dtInformacion = cDataBase.ejecutarConsulta("SELECT Riesgos.Control.IdControl, Riesgos.Control.CodigoControl, " +
                    ...
                    "Riesgos.Control.IdResponsableExperiencia, Riesgos.Control.IdDocumentacion, " +
                    "Riesgos.Control.IdResponsabilidad, Riesgos.Control.IdCalificacionControl, Riesgos.Control.FechaRegistro, " +
                    "Riesgos.Control.IdUsuario, Riesgos.Control.IdMitiga, Parametrizacion.JerarquiaOrganizacional.NombreHijo," +
                    "Riesgos.Control.ResponsableEjecucion, Riesgos.control.Estado " +
                    "FROM Riesgos.Control " +
                    "INNER JOIN Parametrizacion.Test ON Riesgos.Control.IdTest = Parametrizacion.Test.IdTest " +
                    "LEFT JOIN Parametrizacion.JerarquiaOrganizacional " +
                    "ON Parametrizacion.JerarquiaOrganizacional.idHijo = Riesgos.Control.Responsable " +
                    "LEFT JOIN Parametrizacion.DetalleJerarquiaOrg PDJ ON Riesgos.Control.Responsable = PDJ.idHijo " + condicion);
                cDataBase.desconectar();
            }
            catch (Exception ex)
            {
        ...
        public DataTable loadDDLAreas()
        {
            ...
            {
                cDataBase.conectar();
                dtInformacion = cDataBase.ejecutarConsulta("SELECT * FROM Parametrizacion.Area");
                cDataBase.desconectar();
            }
            catch (Exception ex)
            {
  ...
  + "WHERE PVC.Activo = 1 and[IdVariable] = {0}", IdVariable));
        ...
        public int InsertControl(cControlEntity controlEntity)
        {
            ...
            try
            {
                ...
                List<SqlParameter> parametros = new List<SqlParameter>()
                {
                    ...
                    //new SqlParameter() { ParameterName = "@IdClaseControl", SqlDbType = SqlDbType.Int, Value =  controlEntity.Variable1 },
                    new SqlParameter() { ParameterName = "@IdTipoControl", SqlDbType = SqlDbType.Int, Value =  controlEntity.IdTipoControl },
                    //new SqlParameter() { ParameterName = "@IdTipoControl", SqlDbType = SqlDbType.Int, Value =  controlEntity.Variable2 },
                    ...
                    //new SqlParameter() { ParameterName = "@IdResponsableExperiencia", SqlDbType = SqlDbType.Int, Value =  controlEntity.Variable3 },
                    new SqlParameter() { ParameterName = "@IdDocumentacion", SqlDbType = SqlDbType.Int, Value =  controlEntity.IdDocumentacion },
                    //new SqlParameter() { ParameterName = "@IdDocumentacion", SqlDbType = SqlDbType.Int, Value =  controlEntity.Variable4 },
                    new SqlParameter() { ParameterName = "@IdResponsabilidad", SqlDbType = SqlDbType.Int, Value =  controlEntity.IdResponsabilidad },
                    //new SqlParameter() { ParameterName = "@IdResponsabilidad", SqlDbType = SqlDbType.Int, Value =  controlEntity.Variable5 },
            ...
            {
                cDataBase.desconectar();
                mtdUpdateEstado(controlEntity);
            }
        }

        public DataTable mtdUpdateEstado(cControlEntity controlEntity)
        {
            DataTable dtInformacion = new DataTable();
            try
            {
                cDataBase.conectar();
                dtInformacion = cDataBase.ejecutarConsulta("Update Riesgos.Control set estado = 1 where IdControl = " + controlEntity.IdControl);
                cDataBase.desconectar();
            }
            catch (Exception ex)
            {
        ...
        public int InsertControl1(cControlEntity controlEntity)
        {
            #region Variables
            string strConsultaModificacion = string.Empty, strCamposModificacion = string.Empty;
            string strConsultaInsertar = string.Empty, strCamposInsertar = string.Empty;
        ...
        public clsDTOCategoriasVariableControl SeleccionarCategorias(int? Idcategoria)
        {
            ...
            DataTable dt = cDataBase.EjecutarSPParametrosReturnDatatable("[Riesgos].[CategoriaCalificacionSeleccionar2]", parametros);
            if (dt != null && dt.Rows.Count > 0)
            {
                foreach (DataRow Row in dt.Rows)
                {
