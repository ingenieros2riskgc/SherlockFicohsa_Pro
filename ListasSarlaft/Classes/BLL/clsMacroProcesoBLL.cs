...
namespace ListasSarlaft.Classes
{
    public class clsMacroProcesoBLL
    {
        ...
        /// <returns>Retorna si el proceso fue exitoso o no</returns>
        /// 
        public List<clsEstadoPlan> mtdConsultarEstado(ref string strErrMsg)
        {
            #region Vars
            DataTable dtInfo = new DataTable();
            List<clsEstadoPlan> lstMacroProceso = new List<clsEstadoPlan>();
            clsDtMacroproceso cDtMacroproceso = new clsDtMacroproceso();
            clsEstadoPlan objEstadoPlan = new clsEstadoPlan();
            #endregion Vars


            dtInfo = cDtMacroproceso.mtdConsultarEstados(ref strErrMsg);
            ...
            if (dtInfo != null)
            {
                if (dtInfo.Rows.Count > 0)
                {
                    foreach (DataRow dr in dtInfo.Rows)
                    {
                        objEstadoPlan = new clsEstadoPlan(
                            Convert.ToInt32(dr["IdEstadoPlanAccion"].ToString().Trim()),
                            dr["NombreEstadoPlanAccion"].ToString().Trim());

                        lstMacroProceso.Add(objEstadoPlan);
                    }
                }
                else
                {
        ...
        public List<clsRiesgoGlobal> mtdConsultarRiesgosGlobales(ref string strErrMsg)
        {
            #region Vars
            DataTable dtInfo = new DataTable();
            List<clsRiesgoGlobal> lstMacroProceso = new List<clsRiesgoGlobal>();
            clsDtMacroproceso cDtMacroproceso = new clsDtMacroproceso();
            clsRiesgoGlobal objRiesgoGlobal = new clsRiesgoGlobal();
            #endregion Vars


            dtInfo = cDtMacroproceso.mtdConsultarRiesgoGlobal(ref strErrMsg);
            ...
            if (dtInfo != null)
            {
                if (dtInfo.Rows.Count > 0)
                {
                    foreach (DataRow dr in dtInfo.Rows)
                    {
                        objRiesgoGlobal = new clsRiesgoGlobal(
                            Convert.ToInt32(dr["IdRiesgoGlobal"].ToString().Trim()),
                            dr["RiesgoGlobal"].ToString().Trim());

                        lstMacroProceso.Add(objRiesgoGlobal);
                    }
                }
                else
                {
                    lstMacroProceso = null;
                    strErrMsg = "No hay información de Riesgos Globales.";
                }
            }
            else
                lstMacroProceso = null;
