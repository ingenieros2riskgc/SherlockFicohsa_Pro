...
namespace ListasSarlaft.Classes.BLL.Riesgos.Parametrizacion
{
    ...
    {
        public List<clsDTOPlanes> ConsultarPlanes(ref List<clsDTOPlanes> listaPlanes, clsDTOPlanes TodosPlanes, int Transaccion, string estadoPlan)
        {
            #region var
            ...
            dtCarga = planes.ConsultarPlanes(ref TodosPlanes, Transaccion, estadoPlan);
            try
            {
                if (Transaccion == 0)
                {
                ...
                if (Transaccion == 2)
                {
                    if (dtCarga.Rows.Count > 0)
                    {
                        foreach (DataRow dr in dtCarga.Rows)
                        {
                            ...
                            objPlanes.FechaImplementacion = Convert.ToDateTime(dr["FImplementacion"]);
                            objPlanes.FechaExtension = Convert.ToDateTime(dr["FExtension"]);
                            objPlanes.FechaRegistro = Convert.ToDateTime(dr["FechaRegistro"].ToString().Trim());
                            objPlanes.Usuario = dr["Usuario"].ToString().Trim();
