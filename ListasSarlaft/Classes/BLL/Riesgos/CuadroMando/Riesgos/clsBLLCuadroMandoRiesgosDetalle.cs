...
namespace ListasSarlaft.Classes
{
    public class clsBLLCuadroMandoRiesgosDetalle
    {
        ...
        public string LoadInfoDetalleRiesgoInherente(ref string strErrMsg, int IdProbabilidadResidual, int IdImpactoResidual)
        {
            bool booResult = false;
            string NombreRiesgo = string.Empty;
            clsDALCuadroMandoRiesgosDetalle cDALreporte = new clsDALCuadroMandoRiesgosDetalle();
            DataTable dtInfo = new DataTable();
            dtInfo = cDALreporte.LoadInfoDetalleRiesgoInherentes(ref strErrMsg, IdProbabilidadResidual, IdImpactoResidual);
        ...
        public bool LoadInfoReporteRiesgosInherentes(ref string strErrMsg, ref List<clsDTOCuadroMandoRiesgosDetalle> lstInfo, clsDTOCuadroMandoConsolidadoFiltros objFiltros)
        {
            bool booResult = false;
            clsDALCuadroMandoRiesgosDetalle cDALreporte = new clsDALCuadroMandoRiesgosDetalle();
            DataTable dtInfo = new DataTable();
            ...
            string ListaCausas = string.Empty;
            string IdCausasvsControles = string.Empty;
            dtInfo = cDALreporte.LoadInfoReporteRiesgosInherentes(ref strErrMsg, objFiltros);
            if (dtInfo != null)
            {
                if (dtInfo.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dr in dtInfo.Rows)
                    {
                        ...
                        objreporte.strCodigoRiesgo = dr["CodigoRiesgo"].ToString().Trim();
                        objreporte.strNombreRiesgo = dr["NombreRiesgo"].ToString().Trim();
                        objreporte.intIdProbabilidadResidual = Convert.ToInt32(dr["IdProbabilidad"].ToString().Trim());
                        objreporte.intIdImpactoResidual = Convert.ToInt32(dr["IdImpacto"].ToString().Trim());
                        objreporte.strRiesgoInherente = LoadInfoDetalleRiesgoInherente(ref strErrMsg, Convert.ToInt32(dr["IdProbabilidad"].ToString().Trim()), Convert.ToInt32(dr["IdImpacto"].ToString().Trim()));
