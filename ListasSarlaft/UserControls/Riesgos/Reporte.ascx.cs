...
namespace ListasSarlaft.UserControls.Riesgos
{
    public partial class Reporte : System.Web.UI.UserControl
    {
        ...
        private int pagIndexInfoGridRCvsPlanesEvaluacion;
        private int PagIndexInfoGridRCvsPlanesEvaluacion
        {
            get
            {
                pagIndexInfoGridRCvsPlanesEvaluacion = (int)ViewState["pagIndexInfoGridRCvsPlanesEvaluacion"];
                return pagIndexInfoGridRCvsPlanesEvaluacion;
            }
            set
            {
                pagIndexInfoGridRCvsPlanesEvaluacion = value;
                ViewState["pagIndexInfoGridRCvsPlanesEvaluacion"] = pagIndexInfoGridRCvsPlanesEvaluacion;
            }
        }
        ...
        private DataTable infoGridReporteRCvsPlanesEvaluacion;
        private DataTable InfoGridReporteRCvsPlanesEvaluacion
        {
            get
            {
                infoGridReporteRCvsPlanesEvaluacion = (DataTable)ViewState["infoGridReporteRCvsPlanesEvaluacion"];
                return infoGridReporteRCvsPlanesEvaluacion;
            }
            set
            {
                infoGridReporteRCvsPlanesEvaluacion = value;
                ViewState["infoGridReporteRCvsPlanesEvaluacion"] = infoGridReporteRCvsPlanesEvaluacion;
            }
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                mtdLoadAreas();
                mtdLoadProductoAfectado();
                mtdLoadActivoAfectado();
                inicializarValores();
                PopulateTreeView();
                loadCBEstados();
            }
        ...
        private void inicializarValores()
        {
            ...
            PagIndexInfoGridRCvsPlanesEvaluacion = 0;
        }
        ...
        {
            TBResponsable.Text = TreeView3.SelectedNode.Text;
            TBIdResponsable.Text = TreeView3.SelectedNode.Value;
        }
        ...
        private void mtdLoadProductoAfectado()
        {
            try
            {
                DataTable dtInfo = new DataTable();
                dtInfo = cRiesgo.loadDDLProductoAfectado();
                for (int i = 0; i < dtInfo.Rows.Count; i++)
                {
                    DDLProductoAfectado.Items.Insert(i + 1, new ListItem(dtInfo.Rows[i]["Producto"].ToString().Trim(), dtInfo.Rows[i]["IdProductoAfectado"].ToString()));
                }
            }
            catch (Exception ex)
            {
        ...
        private void mtdLoadActivoAfectado()
        {
            try
            {
                DataTable dtInfo = new DataTable();
                dtInfo = cRiesgo.loadDDLActivoAfectado();
                for (int i = 0; i < dtInfo.Rows.Count; i++)
                {
                    DDLActivoAfectado.Items.Insert(i + 1, new ListItem(dtInfo.Rows[i]["Activo"].ToString().Trim(), dtInfo.Rows[i]["IdActivoAfectado"].ToString()));
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                switch (DropDownList1.SelectedValue.ToString().Trim())
                {
                    ...
                    case "9":
                        ...
                        break;
                    case "11":
                        loadGridReporteRCvsPlanesEvaluacion();
                        InfoReporteRCvsPlanesEvaluacion();
                        resetValuesConsulta();
                        ReporteRCvsPlanesEvaluacion.Visible = true;
                        break;
                }
            }
            catch (Exception ex)
        ...
        protected void btnExportarRCvsPlanesEvaluacion_Click(object sender, EventArgs e)
        {
            exportExcel(InfoGridReporteRCvsPlanesEvaluacion, Response, "Reporte Riesgos vs Controles vs Planes de Evaluación");
            //InfoGridReporteControles.ExportToExcel(ExcelFilePath);
        }
        ...
        {
            exportExcel(InfoGridReporteRiesgosPlanesAccion, Response, "Reporte Riesgos vs planes de accion");
        }
        ...
        protected void gvRCvsPlanesEvaluacion_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            PagIndexInfoGridRCvsPlanesEvaluacion = e.NewPageIndex;
            gvRCvsPlanesEvaluacion.PageIndex = PagIndexInfoGridRCvsPlanesEvaluacion;
            gvRCvsPlanesEvaluacion.DataSource = InfoGridReporteRCvsPlanesEvaluacion;
            gvRCvsPlanesEvaluacion.DataBind();
        }
        ...
        private void resetValuesConsulta()
        {
            ...
            trRiesgosCalificacion.Visible = false;
            ...
            ReporteRCvsPlanesEvaluacion.Visible = false;
        ...
        private void loadInfoReporteRiesgos()
        {
            ...
            dtInfo = cRiesgo.ReporteRiesgos(DropDownList52.SelectedValue.ToString().Trim(),
                ...
                cbEstado.SelectedIndex.ToString(), "0", DDLProductoAfectado.SelectedValue.ToString(), DDLActivoAfectado.SelectedValue.ToString());
            ...
            if (dtInfo.Rows.Count > 0)
            {
                ...
                for (int rows = 0; rows < dtInfo.Rows.Count; rows++)
                {
                    ...
                    InfoGridReporteRiesgos.Rows.Add(new Object[] {
                        ...
                        dtInfo.Rows[rows]["FactorRO"].ToString().Trim(),
                        dtInfo.Rows[rows]["Códigos Linea Negocio"].ToString().Trim(),
                        dtInfo.Rows[rows]["Códigos Producto"].ToString().Trim(),
        ...
        private void loadInfoReporteRiesgosCalificacion()
        {
            ...
            dtInfo = cRiesgo.ReporteRiesgos(DropDownList52.SelectedValue.ToString().Trim(),
                ...
                DropDownList2.SelectedValue.ToString().Trim(), DropDownList3.SelectedValue.ToString().Trim(),
                DropDownList4.SelectedValue.ToString().Trim(), "1", "---", DDLareas.SelectedValue.ToString().Trim(),
                cbEstado.SelectedIndex.ToString(), DropDownList52.SelectedItem.ToString().Trim(), DropDownList53.SelectedItem.ToString().Trim());
                ...
                if (dtInfo.Rows.Count > 0)
                {
                    ...
                    for (int rows = 0; rows < dtInfo.Rows.Count; rows++)
                    {
                        if (dtInfo != null)
                        {
                            ...
                            dtInfo.Rows[rows]["ValorEscala"].ToString().Trim(),
        ...
        private void loadGridReporteRiesgosEventos()
        {
            ...
            grid.Columns.Add("ActivoAfectado", typeof(string));
            ...
            if (dtInfo.Rows.Count > 0)
            {
                ...
                for (int rows = 0; rows < dtInfo.Rows.Count; rows++)
                {
                    InfoGridReporteRiesgosEventos.Rows.Add(new Object[] {
                        ...
                        dtInfo.Rows[rows]["ActivoAfectado"].ToString().Trim(),
        ...
        private void loadGridReporteRiesgosPlanesAccion()
        {
            ...
            grid.Columns.Add("NombreArea", typeof(string));
            grid.Columns.Add("ResponsablePlan", typeof(string));
            grid.Columns.Add("ResponsableRiesgo", typeof(string));
            ...
            grid.Columns.Add("EventoFechaCierre", typeof(string));
            grid.Columns.Add("CodigoEvento", typeof(string));
        ...
        private void loadInfoReporteRiesgosPlanesAccion()
        {
            ...
            if (dtInfo.Rows.Count > 0)
            {
                ...
                string fechaExtension = "";
                string fechaImplementacion = "";

                for (int rows = 0; rows < dtInfo.Rows.Count; rows++)
                {
                    fechaExtension = dtInfo.Rows[rows]["FechaExtension"].ToString().Trim();
                    fechaExtension = fechaExtension.Equals("9999-12-31") ? "" : fechaExtension;

                    fechaImplementacion = dtInfo.Rows[rows]["FechaImplementacion"].ToString().Trim();
                    fechaImplementacion = fechaImplementacion.Equals("9999-12-31") ? "" : fechaImplementacion;
                    ...
                    InfoGridReporteRiesgosPlanesAccion.Rows.Add(new Object[] {
                        ...
                        dtInfo.Rows[rows]["NombreArea"].ToString().Trim(),
                        dtInfo.Rows[rows]["ResponsablePlan"].ToString().Trim(),
                        dtInfo.Rows[rows]["ResponsableRiesgo"].ToString().Trim(),
                        ...
                        fechaExtension.Equals("") ? "" : Convert.ToDateTime(fechaExtension).ToString("yyyy-MM-dd"),
                        fechaImplementacion,
                        dtInfo.Rows[rows]["EventoFechaCierre"].ToString().Trim(),
                        dtInfo.Rows[rows]["CodigoEvento"].ToString().Trim(),
        ...
        private void loadGridReporteRCvsPlanesEvaluacion()
        {//yoendyCa
            //RIESGOS
            DataTable grid = new DataTable();
            grid.Columns.Add("CodigoRiesgo", typeof(string));
            ...
            InfoGridReporteRCvsPlanesEvaluacion = grid;
            gvRCvsPlanesEvaluacion.DataSource = InfoGridReporteRCvsPlanesEvaluacion;
            gvRCvsPlanesEvaluacion.DataBind();
        }

        private void InfoReporteRCvsPlanesEvaluacion()
        {
            DataTable dtInfo = new DataTable();
            string ResponsableEjecucion = string.Empty;
            dtInfo = cRiesgo.ReporteRiesgos(DropDownList52.SelectedValue.ToString().Trim(), DropDownList53.SelectedValue.ToString().Trim(),
                    DropDownList54.SelectedValue.ToString().Trim(), DropDownList56.SelectedValue.ToString().Trim(), DropDownList57.SelectedValue.ToString().Trim(),
                    DropDownList2.SelectedValue.ToString().Trim(), DropDownList3.SelectedValue.ToString().Trim(), DropDownList4.SelectedValue.ToString().Trim(), "11", "---",
                    DDLareas.SelectedValue.ToString().Trim(), cbEstado.SelectedIndex.ToString());

            for (int rows = 0; rows < dtInfo.Rows.Count; rows++)
            {
                InfoGridReporteRCvsPlanesEvaluacion.Rows.Add(new Object[] {
                        //RIESGOS
                        dtInfo.Rows[rows]["CodigoRiesgo"].ToString().Trim(),
                        dtInfo.Rows[rows]["NombreRiesgo"].ToString().Trim(),
                        dtInfo.Rows[rows]["CodigoControl"].ToString().Trim(),
                        dtInfo.Rows[rows]["NombreControl"].ToString().Trim(),
                        dtInfo.Rows[rows]["IdPlanEvaluacion"].ToString().Trim(),
