...
namespace ListasSarlaft.UserControls.Riesgos.Gestion
{
    public partial class GestionRiesgoIndicador : System.Web.UI.UserControl
    {
        ...
        protected void mtdStard()
        {
            ...
            if (!mtdLoadRiesgosIndicadores(ref strErrMsg, string.Empty))
                omb.ShowMessage(strErrMsg, 1, "Atención");
            if (!IsPostBack)
                PopulateTreeView();
        }
        #region Metodos Load Riesgos Indicadores
        private bool mtdLoadRiesgosIndicadores(ref string strErrMsg, string Responsable)
        {
            #region Vars
            ...
            #endregion Vars
            lstRiesgoInd = cRiesgoInd.mtdConsultarRiesgosIndicadoresGestion(booResult, ref strErrMsg, Convert.ToInt32(Session["IdUsuario"].ToString()), Responsable);
        ...
        private void mtdShowUpdate(int RowGrid)
        {
            ...
            if (ddlMesMetas.SelectedValue != "0")
            {
                ddlMesMetas.SelectedValue = ((Label)row.FindControl("strMes")).Text;
            }
            if (IdFrecuencia == 3 || IdFrecuencia == 4)
            {
                TrfechasIns.Visible = true;
