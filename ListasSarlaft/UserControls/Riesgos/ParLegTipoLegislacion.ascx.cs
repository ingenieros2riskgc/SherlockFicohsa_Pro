...
namespace ListasSarlaft.UserControls.Riesgos
{
    public partial class ParLegTipoLegislacion : System.Web.UI.UserControl
    {
        ...
        protected void btnAceptarOkNo_Click(object sender, EventArgs e)
        {
            switch (lbldummyOkNo.Text.Trim())
            {
                case "Tipo de legislación":
                    ...
                    {
                        resetValuesCampos();
                        Boolean flag = borrarRegistro();
                        loadGrid();
                        loadInfo();
                        //Mensaje("Tipo de legislación eliminada con éxito.");
                        if (flag == true)
                            omb.ShowMessage("Tipo de legislación eliminada con éxito.", 3, "Atención");
                        else
                            omb.ShowMessage("Error: no se puede eliminar Tipo de Legislación, esta registrado a la Legislación [" + Session["NombreLegislacion"].ToString() + "].", 1, "Atención");
                    }
                    catch (Exception ex)
                    {
                        //Mensaje("Error al eliminar la información. " + ex.Message);
                        omb.ShowMessage("Error al eliminar la información. " + ex.Message.ToString().Trim(), 1, "Atención");
                    }
                    break;
            }
        }
        ...
        protected void ImageButton2_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    omb.ShowMessage("Registro agregado con éxito.", 3, "Atención");
                }
            }
            catch (Exception ex)
            {
                //Mensaje("Error al agregar el registro. " + ex.Message);
                omb.ShowMessage("Error al agregar el registro. " + ex.Message.ToString().Trim(), 1, "Atención");
            }
        }
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (booResult == true)
                    {
                        ...
                        omb.ShowMessage("Registro modificado con éxito.", 3, "Atención");
                    }
                    else
                    {
                        //Mensaje("Error en la actualizacion del registro.");
                        omb.ShowMessage("Error en la actualizacion del registro.", 1, "Atención");
                    }
                }
            }
            catch (Exception ex)
        ...
        {
            Boolean flag = cParametrizacionRiesgos.borrarRegistroTipoLegislacion(InfoGrid.Rows[RowGrid]["IdTipoLegislacion"].ToString().Trim());
            return flag;
        }
