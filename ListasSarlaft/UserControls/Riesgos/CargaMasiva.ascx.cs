...
{ //prueba2 continuacion
    public partial class CargaMasiva : System.Web.UI.UserControl
    {
        ...
        protected void exportExcelEventosCreacion(HttpResponse Response, string filename)
        {
            ...
            cCargaMasivaRCE carga = new cCargaMasivaRCE();
            DataTable FuenteReporte = carga.FuenteReporte();
            FuenteReporte.TableName = "Fuente Reporte";
            DataTable EstadoReporte = carga.EstadoReporte();
            EstadoReporte.TableName = "Estado Reporte";
            DataTable RiesgoGlobal = carga.DataServicio();
            RiesgoGlobal.TableName = "Riesgo Global";
            DataTable CodigoBanco = carga.CodigoBanco();
            CodigoBanco.TableName = "Codigo Banco";
            DataTable Categoria = carga.Categoria();
            Categoria.TableName = "Categoria";
            DataTable Modalidad = carga.LoadDDLModalidad();
            Modalidad.TableName = "Modalidad Ocurrencia";


            DataTable DescripcionLineas = carga.DescripcionLineas();
            DescripcionLineas.TableName = "Descripcion Lineas";

            DataTable DescripcionLineasNivel2 = carga.DescripcionLineasNivel2();
            DescripcionLineasNivel2.TableName = "Descripcion Lineas 2";

            DataTable TipoRiesgo = carga.TipoRiesgo();
            TipoRiesgo.TableName = "Tipo Riesgo";

            DataTable CausaRiesgoUno = carga.CausaRiesgoUno();
            CausaRiesgoUno.TableName = "Causa Riesgo 1";

            DataTable CausaRiesgoDos = carga.CausaRiesgoDos();
            CausaRiesgoDos.TableName = "Causa Riesgo 2";
            DataTable FactorRO = carga.FactorRO();
            FactorRO.TableName = "Factor Ro";

            DataTable ProductoAfectado = carga.ProductoAfectado();
            ProductoAfectado.TableName = "Producto Afectado";

            DataTable LoadDDLOrigen = carga.LoadDDLOrigen();
            LoadDDLOrigen.TableName = "Origen";

            DataTable CuentasPerdida = carga.CuentasPerdida();
            CuentasPerdida.TableName = "Cuentas Perdidas";

            DataTable CriticidadFrecuencia = carga.CriticidadFrecuencia();
            CriticidadFrecuencia.TableName = "CriticidadFrecuencia";

            DataTable LoadDDLsDescSeveridad = carga.LoadDDLsDescSeveridad();
            LoadDDLsDescSeveridad.TableName = "Severidad";

            DataTable LoadDDLEstatus = carga.LoadDDLEstatus();
            LoadDDLEstatus.TableName = "Estatus";
            /*DataTable DataProcesos = carga.DataProcesosRiesgo();
            DataProcesos.TableName = "Procesos 7-16";FactorRO
            DataTable DataClaseRiesgo = carga.DataClaseRiesgo();
            DataClaseRiesgo.TableName = "Clase Riesgo 8-16";ProductoAfectado
            ...
            System.Data.DataSet ds = new System.Data.DataSet();
            ds.Tables.Add(FuenteReporte);
            ds.Tables.Add(EstadoReporte);
            ds.Tables.Add(RiesgoGlobal);
            ds.Tables.Add(CodigoBanco);
            ds.Tables.Add(Categoria);
            ds.Tables.Add(Modalidad);

            ds.Tables.Add(DescripcionLineas);
            ds.Tables.Add(DescripcionLineasNivel2);
            ds.Tables.Add(TipoRiesgo);
            ds.Tables.Add(CausaRiesgoUno);
            ds.Tables.Add(CausaRiesgoDos);
            ds.Tables.Add(FactorRO);

            ds.Tables.Add(ProductoAfectado);
            ds.Tables.Add(LoadDDLOrigen);
            ds.Tables.Add(CuentasPerdida);
            ds.Tables.Add(CriticidadFrecuencia);
            ds.Tables.Add(LoadDDLsDescSeveridad);
            ds.Tables.Add(LoadDDLEstatus);
            /*ds.Tables.Add(DataProcesos);
        ...
        private void LoadGridEventosMasivosCarga(string num)
        {
        ...
        //                            {
        //                                boolEnviarNotificacion(34, Convert.ToInt16("0"), Convert.ToInt32(), "",
        ...
        private void LoadFilePlantillaEventos()
        {
            ...
            if (excelReader != null)
            {
                ...
                for (int i = 0; i < datos.Rows.Count; i++)
                {
                    ...
                    {
                        Linea++;
                        string IdFuenteReporte = datos.Rows[i][0].ToString().Trim();
                        string IdRiesgoGlobal = datos.Rows[i][1].ToString().Trim();
                        string IdEstadoReporte = datos.Rows[i][2].ToString().Trim();
                        string IdCodigoBanco = datos.Rows[i][3].ToString().Trim();
                        string FechaOcurrencia = datos.Rows[i][4].ToString().Trim();
                        string FechaDescubrimiento = datos.Rows[i][5].ToString().Trim();
                        string DescripcionEvento = datos.Rows[i][6].ToString().Trim();
                        string TituloEvento = datos.Rows[i][7].ToString().Trim();
                        string IdCategoria = datos.Rows[i][8].ToString().Trim();
                        string IdModalidadOcurrencia = datos.Rows[i][9].ToString().Trim();
                        string IdFiltroLineaUno = datos.Rows[i][10].ToString().Trim();
                        string IdFiltroDosLineaUno = datos.Rows[i][11].ToString().Trim();
                        string IdFiltroLineaDos = datos.Rows[i][12].ToString().Trim();
                        string IdFiltroDosLineaDos = datos.Rows[i][13].ToString().Trim();
                        string IdTipoRiesgo = datos.Rows[i][14].ToString().Trim();
                        string IdCausaRiesgoN1 = datos.Rows[i][15].ToString().Trim();
                        string IdCausaRiesgoN2 = datos.Rows[i][16].ToString().Trim();
                        string IdFactorRO = datos.Rows[i][17].ToString().Trim();
                        string IdOrigen = datos.Rows[i][18].ToString().Trim();
                        string IdProductoAfectado = datos.Rows[i][19].ToString().Trim();
                        string MontoBruto = datos.Rows[i][20].ToString().Trim().Replace(",", ".");

                        string MontoExposicion = datos.Rows[i][21].ToString().Trim().Replace(",", ".");
                        string NEventos = datos.Rows[i][22].ToString().Trim();
                        string IdCuentasPerdida = datos.Rows[i][23].ToString().Trim();
                        string FechaRegistroPerdida = datos.Rows[i][24].ToString().Trim();
                        string RecuperacionSeguro = datos.Rows[i][25].ToString().Trim();
                        string Recuperaciones = datos.Rows[i][26].ToString().Trim();
                        string CuentaRecuperacion = datos.Rows[i][27].ToString().Trim();

                        string FechaRegistroContable = datos.Rows[i][28].ToString().Trim();
                        string IdCritFrecuenciaG = datos.Rows[i][29].ToString().Trim();
                        string IdCritSeveridadG = datos.Rows[i][30].ToString().Trim();
                        string IdCritFrecuenciaE = datos.Rows[i][31].ToString().Trim();
                        string IdCritSeveridadE = datos.Rows[i][32].ToString().Trim();
                        string IdEstatus = datos.Rows[i][33].ToString().Trim();
                        string FechaCierre = datos.Rows[i][34].ToString().Trim();

                        string Notas = datos.Rows[i][35].ToString().Trim();



                        if (!string.IsNullOrEmpty(FechaOcurrencia.TrimEnd()))
                        {
                            DateTime FechaInicioCm = DateTime.FromOADate(Convert.ToDouble(FechaOcurrencia));
                            FechaOcurrencia = string.Format("{0:yyyy-MM-dd}", Convert.ToDateTime(FechaInicioCm));
                        }

                        if (!string.IsNullOrEmpty(FechaDescubrimiento.TrimEnd()))
                        {
                            DateTime FechaInicioCm = DateTime.FromOADate(Convert.ToDouble(FechaDescubrimiento));
                            FechaDescubrimiento = string.Format("{0:yyyy-MM-dd}", Convert.ToDateTime(FechaInicioCm));
                        }


                        if (!string.IsNullOrEmpty(FechaRegistroPerdida.TrimEnd()))
                        {
                            DateTime FechaInicioCm = DateTime.FromOADate(Convert.ToDouble(FechaRegistroPerdida));
                            FechaRegistroPerdida = string.Format("{0:yyyy-MM-dd}", Convert.ToDateTime(FechaInicioCm));
                        }

                        if (!string.IsNullOrEmpty(FechaRegistroContable.TrimEnd()))
                        {
                            DateTime FechaInicioCm = DateTime.FromOADate(Convert.ToDouble(FechaRegistroContable));
                            FechaRegistroContable = string.Format("{0:yyyy-MM-dd}", Convert.ToDateTime(FechaInicioCm));
                        }


                        if (!string.IsNullOrEmpty(FechaCierre.TrimEnd()))
                        {
                            DateTime FechaInicioCm = DateTime.FromOADate(Convert.ToDouble(FechaCierre));
                        ...
                        try
                        {
                            ...
                            string idEvsEIncs = ModificarEvsEIncs(string.Empty, IdFuenteReporte, IdRiesgoGlobal, IdEstadoReporte, IdCodigoBanco, FechaOcurrencia, FechaDescubrimiento,
                                DescripcionEvento, TituloEvento, IdCategoria, IdModalidadOcurrencia, IdFiltroLineaUno, IdFiltroDosLineaUno, IdFiltroLineaDos, IdFiltroDosLineaDos, IdTipoRiesgo, IdCausaRiesgoN1
                                , IdCausaRiesgoN2, IdFactorRO, IdOrigen, IdProductoAfectado, MontoBruto, MontoExposicion, IdCuentasPerdida, FechaRegistroPerdida, RecuperacionSeguro, Recuperaciones, CuentaRecuperacion, FechaRegistroContable,
                               NEventos, IdCritFrecuenciaG, IdCritSeveridadG, IdCritFrecuenciaE, IdCritSeveridadE, IdEstatus, FechaCierre, Notas, string.Empty);

                            try
                            {
                                s.Append("El Evento o Insidente en la linea: " + i + " Se inserto Correctamente con id:" + idEvsEIncs); s.Append(Constantes.TAB);

                                writer.WriteLine(s.ToString());
                                ContadorOk++;
                                descripcionEvento += " <br /> <B> " + idEvsEIncs + ", </B> ";
                            }
                            catch (Exception ex)
                            {
                                Mensaje("Error al registrar el evento de la linea: " + i + "." + ex.Message);
                                s.Append("El Evento o Insidente en la linea: " + i + "Genero error, por favor validar. "); s.Append(Constantes.TAB);
                                writer.WriteLine(s.ToString());
                                ContadorBad++;
            ...
            if (excelReader != null)
            {
                ...
                for (int i = 1; i < datos.Rows.Count; i++)
                {
                    if (datos.Rows[i][0].ToString().Trim() != "")
                    {
                        ...
                        string estado = datos.Rows[i][21].ToString().Trim(); //Columna V
                        ...
                        string Responsable = datos.Rows[i][32].ToString().Trim();
                        ...
                        try
                        {
                            ...
                            Porcentaje_y_un, Impacto_cualitativo, Pérdida_económica_entre, y, tratamiento, IdUsuario, FechaRegistro, estado, Responsable);
                    ...
                    foreach (IXLRow dataRow in nonEmptyDataRows)
                    {
                        if (dataRow.RowNumber() > 1)
                        {
                            if (tipoCarga == "2")
                            {
                                ...
                                controlEntity.Variable1 = ValidarNumero(dataRow.Cell(9).Value.ToString());
                                controlEntity.Variable2 = ValidarNumero(dataRow.Cell(10).Value.ToString());
                                controlEntity.Variable3 = ValidarNumero(dataRow.Cell(11).Value.ToString());
                                controlEntity.Variable4 = ValidarNumero(dataRow.Cell(12).Value.ToString());
                                controlEntity.Variable5 = ValidarNumero(dataRow.Cell(13).Value.ToString());
                                ...
                                controlEntity.Variable15 = ValidarNumero(dataRow.Cell(22).Value.ToString());
                                ...
                                cControl.InsertControl1(controlEntity);
                                //  -yoendy
        ...
        #region masivo eventosincidentes
