...
namespace ListasSarlaft.UserControls.Riesgos
{
    public partial class CalificacionExpPuntosDeCorte : System.Web.UI.UserControl
    {
        ...
        {
            int Transaccion = 8;
            if (ModalMin.Text == string.Empty)
            {
                omb.ShowMessage("Error: Valor minimo vacío! ", 1, "Atención");
                return;
            }
            if (ModalMax.Text == string.Empty)
            {
                omb.ShowMessage("Error: Valor maximo vacío! ", 1, "Atención");
                return;
            }
