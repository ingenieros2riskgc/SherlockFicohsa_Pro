...
namespace ListasSarlaft.UserControls.MAuditoria
{
    public partial class GruposAuditoria : System.Web.UI.UserControl
    {
        ...
        private DataTable GetTreeViewData()
        {
            ...
            string selectCommand = "SELECT distinct IdHijo,IdPadre,NombreHijo,TipoArea FROM [Parametrizacion].[JerarquiaOrganizacional]";
        ...
        private string DetalleNodo(int tipoSelect, string idHijo)
        {
            ...
            if (tipoSelect == 1)
                selectCommand = "SELECT distinct NombreResponsable,CorreoResponsable FROM [Parametrizacion].[DetalleJerarquiaOrg] WHERE idHijo = " + idHijo;
            else
                selectCommand = "SELECT distinct NombreResponsable,CorreoResponsable, NombreHijo FROM [Parametrizacion].[JerarquiaOrganizacional] LEFT OUTER JOIN [Parametrizacion].[DetalleJerarquiaOrg] ON [DetalleJerarquiaOrg].idHijo = [JerarquiaOrganizacional].idHijo WHERE [JerarquiaOrganizacional].idHijo = " + idHijo;
        ...
        {
            // Get JerarquiaOrganizacional table
            string selectCommand = "SELECT distinct IdHijo,IdPadre,NombreHijoAuditoria FROM [Auditoria].[JerarquiaGrupoAuditoria] WHERE idGrupoAuditoria = " + lblIdGrupo.Text;
            string conString = WebConfigurationManager.ConnectionStrings["SarlaftConnectionString"].ConnectionString;
            SqlDataAdapter dad = new SqlDataAdapter(selectCommand, conString);
            DataTable dtblDiscuss = new DataTable();
            dad.Fill(dtblDiscuss);
