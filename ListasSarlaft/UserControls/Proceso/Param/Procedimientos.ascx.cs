...
namespace ListasSarlaft.UserControls.Proceso
{
    public partial class Procedimientos : System.Web.UI.UserControl
    {
        ...
        private DataTable infoGrid8;
        private int rowGrid8;
        private int pagIndex8;
        ...
        private DataTable InfoGrid8
        {
            get
            {
                infoGrid8 = (DataTable)ViewState["infoGrid8"];
                return infoGrid8;
            }
            set
            {
                infoGrid8 = value;
                ViewState["infoGrid8"] = infoGrid8;
            }
        }
        ...
        private int RowGrid8
        {
            get
            {
                rowGrid8 = (int)ViewState["rowGrid8"];
                return rowGrid8;
            }
            set
            {
                rowGrid8 = value;
                ViewState["rowGrid8"] = rowGrid8;
            }
        }
        ...
        private int PagIndex8
        {
            get
            {
                pagIndex8 = (int)ViewState["pagIndex8"];
                return pagIndex8;
            }
            set
            {
                pagIndex8 = value;
                ViewState["pagIndex8"] = pagIndex8;
            }
        }
        ...
        {
            //RowGrid8 = (Convert.ToInt16(GridView8.PageSize) * PagIndex8) + Convert.ToInt16(e.CommandArgument);
            RowGrid8 = Convert.ToInt16(e.CommandArgument);
            switch (e.CommandName)
            {
                case "Relacionar":
                    tbxActividad.Text = InfoGrid8.Rows[RowGrid8][1].ToString().Trim();
                    lblIdActividad.Text = InfoGrid8.Rows[RowGrid8][0].ToString().Trim();
        ...
        /// </summary>
        /// <param name="strErrMsg">Mensaje de error</param>
        private bool mtdLoadInfoGridActividades(ref string strErrMsg)
        {
            bool booResult = false;
        ...
        {
            RowGrid2 = e.NewPageIndex;
            GridView2.PageIndex = PagIndex2;
            GridView2.DataSource = InfoGrid2;
            GridView2.DataBind();
        ...
        protected void ImageButton25_Click(object sender, ImageClickEventArgs e)
        {
            ...
            {
                string strErrMsg = "";
                mtdLoadGridActividadess();
                loadInfoConsultarActividadess();
                GridView8.Visible = true;

            }
            catch (Exception ex)
            {
                lblMsgBox.Text = ("Error al realizar la consulta. " + ex.Message);
            }
        }

        private void mtdLoadGridActividadess()
        {
            DataTable grid = new DataTable();
            ...
            grid.Columns.Add("CargoResponsable", typeof(string));
            ...
            GridView8.DataSource = grid;
            GridView8.DataBind();
            InfoGrid8 = grid;
        }

        private void loadInfoConsultarActividadess()
        {
            List<clsActividad> lstActividad = new List<clsActividad>();
            clsActividadBLL cActividad = new clsActividadBLL();
            DataTable dtInfo = new DataTable();
            dtInfo = cActividad.mtdConsultarActividadActivass();
            if (dtInfo.Rows.Count > 0)
            {
                for (int rows = 0; rows < dtInfo.Rows.Count; rows++)
                {
                    InfoGrid8.Rows.Add(new object[] { dtInfo.Rows[rows]["Id"].ToString().Trim(),
                                                                       dtInfo.Rows[rows]["Descripcion"].ToString().Trim(),
                                                                       dtInfo.Rows[rows]["IdCargoResponsable"].ToString().Trim(),
                                                                       dtInfo.Rows[rows]["CargoResponsable"].ToString().Trim(),
                ...
                GridView8.DataSource = InfoGrid8;
                GridView8.DataBind();
            }
            else
            {
        ...
        {
            //RowGrid8 = e.NewPageIndex;
            //GridView8.PageIndex = PagIndex8;
            //GridView8.DataSource = InfoGrid8;
            //GridView8.DataBind();

            PagIndex8 = e.NewPageIndex;
            GridView8.PageIndex = PagIndex8;
            GridView8.DataSource = InfoGrid8;
            GridView8.DataBind();
