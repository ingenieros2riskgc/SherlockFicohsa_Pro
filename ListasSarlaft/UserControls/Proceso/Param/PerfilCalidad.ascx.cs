...
namespace ListasSarlaft.UserControls.Proceso
{
    public partial class PerfilCalidad : System.Web.UI.UserControl
    {
        ...
        private void CargarEstadosPerfil()
        {
            ...
            try
            {
                dtInfo = cRiesgo.mtdLoadEstadoPerfil(true);
                ddlEstadoPerfil.Items.Clear();
                ddlEstadoPerfil.Items.Insert(0, new ListItem("", "0"));
                int intCounter = 1;
                for (int i = 0; i < dtInfo.Rows.Count; i++)
                {
                    //ddlEstadoPerfil.Items.Insert(i + 1, new ListItem(dtInfo.Rows[i]["NombreEstadoDocumento"].ToString(), dtInfo.Rows[i]["IdEstadoDocumento"].ToString()));
                    ddlEstadoPerfil.Items.Insert(intCounter, new ListItem(dtInfo.Rows[i]["NombreEstadoDocumento"].ToString(), dtInfo.Rows[i]["IdEstadoDocumento"].ToString()));
                    intCounter++;
                }
            }
            catch (Exception ex)
