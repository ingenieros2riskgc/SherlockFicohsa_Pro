...
namespace ListasSarlaft.UserControls.ConfigEstructura
{
    public partial class ConfEstructuraArchivo : System.Web.UI.UserControl
    {
                ...
                else
                {
                    if (ddlTipoParametro.SelectedIndex != 0)
                    {
                        ...
                        mtdAgregarEstructuraCampo(Session["IdUsuario"].ToString().Trim(), string.Empty, Sanitizer.GetSafeHtmlFragment(tbNombreCampo.Text.Trim()), "1", "0", chbEsParametrico.Checked,
                            ddlTipoParametro.SelectedValue.ToString().Trim(), string.Empty, string.Empty, Sanitizer.GetSafeHtmlFragment(tbPosicion.Text.Trim()), cbEstado.Checked, cbNumerico.Checked, ref strErrMsg);
                        ...
                        if (string.IsNullOrEmpty(strErrMsg))
                        {
                            ...
                            ShowMessage("La estructura fue creada exitósamente.", 3, "Atención");
                        }
                        else
            ...
            try
            {
                ...
                else
                {
                    ...
                    mtdActualizarEstructura(Session["IdUsuario"].ToString().Trim(), InfoGrid.Rows[RowGrid]["StrIdEstructCampo"].ToString().Trim(),
                        ...
                        string.Empty, string.Empty, Sanitizer.GetSafeHtmlFragment(tbPosicion.Text.Trim()), cbEstado.Checked, cbNumerico.Checked, ref strErrMsg);
                    ...
                    if (string.IsNullOrEmpty(strErrMsg))
                    {
                        ...
                        ShowMessage("La estructura fue actualizada exitósamente.", 3, "Atención");
                    }
                    else
                        mtdMensaje(strErrMsg);
                }
        ...
        {
            clsParamArchivo cParamArchivo = new clsParamArchivo();
            clsDTOEstructuraCampoEstructura objTipoParamIn = new clsDTOEstructuraCampoEstructura(strIdUsuario, strIdEstructCampo, strNombreCampo, strIdTipoDato, strLongitud,
        ...
        {
            clsParamArchivo cParamArchivo = new clsParamArchivo();
            clsDTOEstructuraCampoEstructura objTipoParamIn = new clsDTOEstructuraCampoEstructura(strIdUsuario, strIdEstrucCampo, strNombreCampo, strIdTipoDato, strLongitud,
                booEsParametrico, strIdTipoParametro, strNombreTipoParametro, strNombreTipoDato, strPosicion, booEstado, booNumerico);
