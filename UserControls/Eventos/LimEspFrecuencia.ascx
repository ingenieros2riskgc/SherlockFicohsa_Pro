<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="LimEspFrecuencia.ascx.cs" Inherits="ListasSarlaft.UserControls.Eventos.LimEspFrecuencia" %>
...
<asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:SarlaftConnectionString %>"
    DeleteCommand="DELETE FROM [Eventos].[MtEspFrecuencia] WHERE IdMtEspFrecuencia = @IdMtEspFrecuencia"
    InsertCommand="INSERT INTO [Eventos].[MtEspFrecuencia] ([IdDescFrecuencia], [IdTipoRiesgo], [Minimo], [Maximo], [IdUsuario], [FechaRegistro]) VALUES (@IdDescFrecuencia, @IdTipoRiesgo, @Minimo, @Maximo, @IdUsuario, @FechaRegistro)"
    SelectCommand="SELECT EMF.IdMtEspFrecuencia, ETR.IdTipoRiesgo, ETR.TipoRiesgo, EDF.IdDescFrecuencia, EDF.DescripcionFicohsa, Minimo, Maximo, LU.Nombres AS Usuario, EMF.FechaRegistro FROM [Eventos].[MtEspFrecuencia] EMF INNER JOIN Eventos.DescFrecuencia  EDF ON EMF.IdDescFrecuencia = EDF.IdDescFrecuencia INNER JOIN Eventos.TipoRiesgo ETR ON ETR.IdTipoRiesgo = EMF.IdTipoRiesgo INNER JOIN [Listas].[Usuarios] LU ON LU.IdUsuario = EMF.IdUsuario ORDER BY IdTipoRiesgo, Minimo"
    UpdateCommand="UPDATE [Eventos].[MtEspFrecuencia] SET [IdDescFrecuencia] = @IdDescFrecuencia, [IdTipoRiesgo] = @IdTipoRiesgo, [Minimo] = @Minimo, [Maximo] = @Maximo  WHERE IdMtEspFrecuencia = @IdMtEspFrecuencia">
    <DeleteParameters>
        <asp:Parameter Name="IdMtEspFrecuencia" Type="Int32" />
    </DeleteParameters>
    <InsertParameters>
    ...
    <ContentTemplate>
        ...
        <table align="center">
            <tr align="center" bgcolor="#002649">
                <td>
                    <%--<br />--%>
                    <asp:Label ID="Label13" runat="server" ForeColor="White" Text="Límites Específicos Frecuencia" Font-Bold="False"
                        ...
                        <tr>
                            <td>
                                <asp:GridView ID="GridView1" runat="server" CellPadding="4" DataSourceID="SqlDataSource1"
                                    ...
                                    DataKeyNames="IdMtEspFrecuencia, IdTipoRiesgo, IdDescFrecuencia, Minimo, Maximo" OnSelectedIndexChanged="GridView1_SelectedIndexChanged"
                                    ...
                                    <Columns>
                                        <asp:BoundField DataField="IdMtEspFrecuencia" HeaderText="Código" InsertVisible="False"
                                            ReadOnly="True" SortExpression="IdMtEspFrecuencia" Visible="True">
